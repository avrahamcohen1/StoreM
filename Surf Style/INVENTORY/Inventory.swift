...
class Inventory: UIViewController,  UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIPickerViewDataSource, UIPickerViewDelegate{
    ...
    func readSqlData(filter:String){
        ...
        let ipPort = getSettingData(key:"localIP") + ":" + getSettingData(key:"localPort")
        client.disconnect()
        client.connect(ipPort, username: "sa", password: getSettingData(key:"localPass"), database: getSettingData(key:"localDb")) { success in
...
extension Inventory: UITextFieldDelegate {
